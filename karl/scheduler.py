...
from karl.util import ScheduleRequest, Params, Fact, User, History, theme_fs
...
class MovingAvgScheduler:
    ...
    def reset_fact(self, fact_id=None):
        # delete fact from db
    ...
    def embed(self, facts: List[Fact]):
    ...
    def get_fact(self, request: ScheduleRequest) -> Fact:
        '''get fact from db, insert if new'''
    ...
    def get_facts(self, requests: List[ScheduleRequest]) -> List[Fact]:
        ...
        logger.debug('embed facts ' + str(len(new_facts)))
        self.embed(new_facts)
        fact_skills = self.get_fact_skills(new_facts)
    ...
    def get_fact_skill(self, fact: Fact) -> float:
    ...
    def get_fact_skills(self, facts: List[Fact]) -> List[float]:
        return [self.get_fact_skill(fact) for fact in facts]
    ...
    def dist_recall_batch(self, user: User, facts: List[Fact]) -> float:
    ...
    def get_rationale(self, user, facts, date, scores, order, top_n_facts=3) -> str:
        rr = """
             <style>
             table {
    ...
    def get_facts_info(self, user, facts, date, scores, order, top_n_facts=3) -> dict:
        facts_info = []
        for i in order[:top_n_facts]:
    ...
    def score_user_facts(self, user: User, facts: List[Fact], date: datetime, plot=False) -> Dict:
        ...
        if plot:
            ...
            self.plot_histogram(fact.qrep, user_qrep, local_filename)
    ...
    def schedule(self, requests: List[ScheduleRequest], date: datetime, plot=False) -> dict:
        ...
        t0 = datetime.now()
        logger.debug('scheduling {} facts'.format(len(requests)))
        ...
        if not self.params.precompute:
            return self.score_user_facts(user, facts, date, plot=plot)
        t1 = datetime.now()
        schedule_profile['score_user_facts (wo pre)'] = (len(facts), t1 - t0)
        ...
        if user.user_id not in self.precompute_commit:
            ...
            t0 = datetime.now()
            output_dict = self.score_user_facts(user, facts, date, plot=plot)
            t1 = datetime.now()
            schedule_profile['score_user_facts (pre not found)'] = (len(facts), t1 - t0)
            ...
            output_dict = self.score_user_facts(user, facts, date, plot=plot)
            t1 = datetime.now()
            schedule_profile['score_user_facts (pre marked done)'] = (len(facts), t1 - t0)
        else:
            ...
            if len(new_facts) > 0:
                new_results = self.score_user_facts(user, new_facts, date, plot=plot)
            ...
            t1 = datetime.now()
            schedule_profile['score_user_facts (new facts)'] = (len(new_facts), t1 - t0)
    ...
    def branch(self, user: User, facts: List[Fact],
               date: datetime, fact_idx: int,
               response: str, plot=False) -> None:
        # make copy of user and top fact
        # update (copied) user and top fact by response
        # make new schedule with updated user and facts (with updated fact)
        ...
        # next fact. but it should affect all facts (with repetition) equally
        self.update_user_fact(user, facts[fact_idx], date, response)
        results = self.score_user_facts(user, facts, date, plot=plot)
        results.update({
            'user': user,
            'fact': facts[fact_idx],
            'facts': facts,
    ...
    def update_user_fact(self, user: User, fact: Fact, date: datetime, response: str):
        # update qrep
    ...
    def leitner_update(self, user: User, fact: Fact, response: str):
        # leitner boxes 1~10
    ...
    def sm2_update(self, user: User, fact: Fact, response: str):
    ...
    def plot_histogram(self, fact_qrep, user_qrep, filename):
